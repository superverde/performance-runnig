...
import type { AffiliateStats } from '@/lib/affiliate-stats'

/**
 * So a apresentacao. Os dados vem ja calculados do servidor (page.tsx) — este
 * componente nunca conhece a INTERNAL_API_KEY, ao contrario da versao anterior,
 * que a trazia escrita no codigo e portanto dentro do bundle enviado ao browser.
 */
export function AfiliadosClient({ data }: { data: AffiliateStats }) {
  const router = useRouter()

  const totalAll = data.ranking.reduce((s, r) => s + r.total, 0)
  const totalToday = data.today.reduce((s, r) => s + r.count, 0)
  const maxDay = Math.max(...data.daily.map((d) => d.clicks), 1)
